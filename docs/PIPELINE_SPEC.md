...
```bash
0 7 * * * cd /path/to/state-zero && python3 src/scripts/pipeline.py >> /var/log/state-zero.log 2>&1
```
...
with open('output/daily_data.json') as f:
    data = json.load(f)
with open('output/interpretation.txt') as f:
    interpretation = f.read().strip()
